...
export function buildShapeData( shapes, tileBounds, options = {} ) {
	...
	for ( const [ id, shape ] of shapes ) {
		...
		if ( cat === 'point' ) {
			...
			const hwVal = sw * pxDeg / 2;
			const sa = arrowInt( opts.startArrowStyle );
			const ea = arrowInt( opts.endArrowStyle );
			const aszVal = ( opts.arrowSize || 0 ) * pxDeg;

			let dashVal = 0, gapVal = 0;
			...
			arr.push( 3, total, lineColor[ 0 ], lineColor[ 1 ], lineColor[ 2 ], lineColor[ 3 ], 0, 0, 0, 0, 0, op, vc, hwVal, sa, ea, aszVal, dashVal, gapVal );
